...
mod keys {
    ...
    fn alice_and_bob() {
        ...
        let alice_shared_secret = alice_private.dh(&bob_public);
        let bob_shared_secret = bob_private.dh(&alice_public);

        assert_eq!(alice_shared_secret.as_bytes(), bob_shared_secret.as_bytes());
    }

    fn do_rfc7748_ladder_test1(input_scalar: &[u8], input_point: &[u8], expected: [u8; 32]) {
        let private = PrivateKey::from_bytes(input_scalar);
        assert_eq!(private.is_ok(), true);
        let private = private.unwrap();

        let public = PublicKey::from_bytes(input_point);
        assert_eq!(public.is_ok(), true);
        let public = public.unwrap();

        let result = private.dh(&public).to_bytes();
        assert_eq!(result, expected);
    }

    fn x25519(input_scalar: [u8; 32], input_point: [u8; 32]) -> [u8; 32] {
        let private = PrivateKey::from_bytes(&input_scalar).unwrap();
        let public = PublicKey::from_bytes(&input_point).unwrap();
        private.dh(&public).to_bytes()
    }
